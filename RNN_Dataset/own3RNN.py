...
df_both = get_df()
ids = list(set([int(id.split('_')[0]) for id in list(set(df_both.ID))]))
...
def create_and_test_model(df_ini, ids, combination_id):
    ...
    train_ids, valid_ids, test_ids = ids
    train = df_ini[~df_ini.ID.isin(valid_ids + test_ids)].drop(['ID'], axis=1)
    valid = df_ini[df_ini.ID.isin(valid_ids)].drop(['ID'], axis=1)
    test = df_ini[df_ini.ID.isin(test_ids)].drop(['ID'], axis=1)
    ...
    for var in ['train_ids', 'valid_ids', 'test_ids', 'train', 'valid', 'test']:
        exec(f'del {var}')
    ...
    with open('results/{}/{}.txt'.format(FOLDER, name), 'w') as f:
        for i, score in enumerate([z_leg_scores, y_leg_scores, x_leg_scores, z_arm_scores, y_arm_scores, x_arm_scores]):
            f.write("{}: {}\n".format(score_id_name[i], score))
...
for i in range(k_fold + 1):
    ...
    seed(i * 10 + 12)
    valid_ids = sample(list(set(train_ids) - set(test_ids)), floor(len(train_ids)*0.1))
...
'''
for i, combination in enumerate([[11, 12], [11, 14], [11, 15], [12, 14], [12, 15], [14, 15]]):
    
    train_ids = combination
    test_ids = list(set(ids).difference(set(train_ids)))
    seed(i)
    validation_ids = sample(test_ids, 1)
    test_ids.remove(validation_ids[0])

    train_ids, validation_ids, test_ids = [[str(id) for id in ids] for ids in [train_ids, validation_ids, test_ids]]

    create_and_test_model(df_both, [train_ids, validation_ids, test_ids], i)
'''
