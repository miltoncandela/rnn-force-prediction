...
def df_to_generator(df):
    ...
    df_generator = TimeseriesGenerator(data=np.array(df).astype(np.float16), targets=np.array(df_output).astype(np.float16), length=SEQ_SIZE, batch_size=n_features)
